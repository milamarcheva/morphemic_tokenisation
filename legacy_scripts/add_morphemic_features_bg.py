...
def spacy_tokenise(doc, tok_ann=None):
  sent = []
  postags_morphtok = [] 
  morlvl_postags_morpthok = []
  ...
  for idx, t in enumerate(doc):
    ...
    #regular plurals (-s)
    elif t.tag_ in plural_tags and t.lower_[-1] == 's' and t.lower_ not in s_exceptions:
      morpheme = t.lower_[-1:]
      lemma = ""
...
def process_row(row, nlp, text_col: str, tok_ann_col: str):
    ...
    doc = nlp(utt)  # always parse for features
...
def main():
    ...
    # mask = out_df["sent_morphtok"].fillna("").str.contains(r"\S+\s+\S")
    # out_df = out_df.loc[mask].reset_index(drop=True)

    mask = (
        out_df["sent_morphtok"]
        .fillna("")
        .str.contains(r"\S+\s+\S")          # at least two tokens
        & ~out_df["sent_morphtok"].str.contains(r"\b(?:xxx|yyy)\b", case=False)
    )
